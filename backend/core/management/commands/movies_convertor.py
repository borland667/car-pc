...
class Command(BaseCommand):
    ...
    def _convert(self, file_path):
        ...
        command = '%s %s --sout "#transcode{vcodec=h264,vb=800,scale=1,width=320,height=240,acodec=mpga,ab=128,channels=2,samplerate=44100}:std{access=file,mux=mp4,dst=%s}" vlc://quit' % (
            settings.VLC_BIN,
            file_path,
            move_path
        )
